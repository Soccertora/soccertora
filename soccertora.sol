...
/**
Soccer Tora
https://t.me/soccertora
https://soccertora.net/
*/
...
contract SoccerTora is Context, IERC20, Ownable {
    ...
    string private constant _name = "Soccer Tora";
    ...
    uint256 public _maxSellAmount = 10000000  * 10**_decimals;
    ...
    function changeFee(uint256 _buyMarketingFee, uint256 _buyLiquidityFee, uint256 _buyBurnFee, uint256 _sellMarketingFee, uint256 _sellLiquidityFee, uint256 _sellBurnFee) public onlyOwner {
        require(_buyMarketingFee + _buyLiquidityFee + _buyBurnFee < 25 || _sellLiquidityFee + _sellMarketingFee + _sellBurnFee < 25, "Can't change fee higher than 24%");
    ...
    function changeMaxPercent(uint256 _maxBuyPercent, uint256 _maxSellPercent, uint256 _maxWalletPercent) public onlyOwner {
        require(_maxBuyPercent + _maxSellPercent + _maxWalletPercent >= 1, "Should be bigger than 0,1%");
        _maxBuyAmount = (_tTotal * _maxBuyPercent) / 100;
        _maxSellAmount = (_tTotal * _maxBuyPercent) / 100;
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if(to == uniswapV2Pair && from != uniswapV2Pair){
                require(amount <= _maxSellAmount, "Transfer Amount exceeds the maxTxnsAmount");
    ...
    function removeExternalTokens(address tokenAddress, uint256 amount) public onlyOwner {
        require(tokenAddress != address(this), "can't withdraw native token");
